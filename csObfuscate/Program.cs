	...
	class Program 
	{
		...
		private void Run(Arguments arguments)
		{
			try
			{
				...
				{
					var config = arguments.Options["config"].Value;
				...
				foreach(var file in arguments.Parameters)
				{
		...
		private void Obfuscar(string file, string signatureFile, bool testLoad, bool debug, bool embed = false)
		{
			...
			if (debug)
			{
				foreach (var line in result.Combined.SplitNewLine())
				{
		...
		private void ObfucatorComplete(string sourceDir, string file, bool embed, bool testLoad)
		{
			...
			var outFile = FileSystem.Combine(sourceDir, "tmp", Path.GetFileName(file));
			if (testLoad)
			{
				Stopwatch watch = Stopwatch.StartNew();
			...
			if (File.Exists(originalPdb)) File.Delete(originalPdb);

			string newPdb = FileSystem.Combine(sourceDir, "tmp", Path.ChangeExtension(outFile, ".pdb"));
			if (File.Exists(newPdb)) File.Move(newPdb, originalPdb);
